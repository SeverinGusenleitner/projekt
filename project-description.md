<<<<<<< HEAD
# **Website Project Description**

## **1. Project Title**

* Give your website a clear and creative title.
    - Matura cars
---

## **2. Project Idea**

* Describe what your website is about.

  * What content will it include? 
    - best starter vehicle from chosen brand
  * Why is this topic interesting or useful? 
    - useful for students who dont have alot of money

---

## **3. Target Audience**

* Who do you want to visit and use your website?

  * Describe your main **target audience** (age group, interests, needs).
    - age: 17-20 years old (people who just got their drives license)
    - needs: cheap car of a specific brand

  * Why will your website be interesting or helpful for them?
    - because the website doesnt just give them the cheapest car, but they can also select the brand and stuff

---

## **4. Structure of the Website**

* How many pages (HTML files) will your website have?
* What will each page be about?
  *Example:*

  * `index.html` → Home page
  * `bmw.html` → bmw page
  * `ford.html` → ford page
  * ...
---

## **5. Design and Layout**

* Describe your planned design:

  * Which colors, fonts, and shapes will you use? 
    * simple white a and black page
    * contrast color(maybe dark blue)
    * rounded borders
    * font: arial, sans-serif;
    * minimal design
  * How will your navigation menu look?
    * sidebar menu with all the cars.
    ![sidebar](sidebar.png )
    * once you click on one brand you get to another HTML page
  * Which parts will stay the same on all pages 
    * CSS styling

---

## **6. Milestones**

* Plan the main steps for your project.
  *Example:*

  * **Week 1:** Planning and creating the structure
  * **Week 2:** Building index.html and sidebar Styling with CSS
  * **Week 3:** building subpages for each car brand with about and stuff
  * **Week 4:** Testing and final presentation

---

## **7. Special Features (optional)**

* Is there something special you would like to try out?
* flexbox for the cars on each page when you click on a div it should become big and cover like the whole screen

=======
# **Website Project Description**

## **1. Project Title**

* Give your website a clear and creative title.
    - Matura cars
---

## **2. Project Idea**

* Describe what your website is about.

  * What content will it include? 
    - best starter vehicle from chosen brand
  * Why is this topic interesting or useful? 
    - useful for students who dont have alot of money

---

## **3. Target Audience**

* Who do you want to visit and use your website?

  * Describe your main **target audience** (age group, interests, needs).
    - age: 17-20 years old (people who just got their drives license)
    - needs: cheap car of a specific brand

  * Why will your website be interesting or helpful for them?
    - because the website doesnt just give them the cheapest car, but they can also select the brand and stuff

---

## **4. Structure of the Website**

* How many pages (HTML files) will your website have?
* What will each page be about?
  *Example:*

  * `index.html` → Home page
  * `bmw.html` → bmw page
  * `ford.html` → ford page
  * ...
---

## **5. Design and Layout**

* Describe your planned design:

  * Which colors, fonts, and shapes will you use? 
    * simple white a and black page
    * contrast color(maybe dark blue)
    * rounded borders
    * font: arial, sans-serif;
    * minimal design
  * How will your navigation menu look?
    * sidebar menu with all the cars.
    ![sidebar](sidebar.png )
    * once you click on one brand you get to another HTML page
  * Which parts will stay the same on all pages 
    * CSS styling

---

## **6. Milestones**

* Plan the main steps for your project.
  *Example:*

  * **Week 1:** Planning and creating the structure
  * **Week 2:** Building index.html and sidebar Styling with CSS
  * **Week 3:** building subpages for each car brand with about and stuff
  * **Week 4:** Testing and final presentation

---

## **7. Special Features (optional)**

* Is there something special you would like to try out?
* flexbox for the cars on each page when you click on a div it should become big and cover like the whole screen

>>>>>>> aed9259ad61cab5c3958091da2544e31ead18789
---